  ...
  forEach(fragments, (fragment, key) => {
    ...
    if (vars) {
      forEach(vars, (varValue, varKey) => {
        if (varValue.type && varValue.value && typeof varValue.type === 'string') {
...
export function buildQueryAndVariables (fragments, inputVariables = {}) {
  variables = {};
  variablesTypes = [];
  iterator = 0;
  ...
  if (variablesTypes.length) {
    query = `query (${variablesTypes.join(',')}) { ${queries} }`;
  } else {
    query = `query { ${queries} }`;
  }
